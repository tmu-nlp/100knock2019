...
with open('result_47.txt', 'w', encoding='utf-8') as o_file:
        ...
        for chunk in chunks:
            ...
            if chunk.has_pos('動詞'):
                ...
                for src in chunk.srcs:
                    if chunks[src].get_sahensetuzoku_wo() != '':
                        sahen_wo_list.append(
                            chunks[src].get_sahensetuzoku_wo())

                if len(sahen_wo_list) >= 1:
                    for sahen_wo in sahen_wo_list:
                        predicate = sahen_wo + \
                            chunk.get_allsurfaces_base('pos', '動詞')[0]
                        particles = []
                        phrases = []
                        pairs = []
                        for src in chunk.srcs:
                            if len(chunks[src].get_allsurfaces_base('pos', '助詞')) > 0:
                                if chunks[src].get_allsurfaces_base('pos', '助詞').pop() != 'を':
                                    particles.append(
