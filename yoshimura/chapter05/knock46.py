...
この文は「始める」と「見る」の２つの動詞を含み，「始める」に係る文節は「ここで」，「見る」
に係る文節は「吾輩は」と「ものを」と解析された場合は，次のような出力になるはずである．
...
for chunks in get_chunk_list("neko.txt.cabocha"):
    for chunk in chunks:
        ...
        predicate = chunk.get_surfaces('pos', '動詞')[0]  # 述語
        case_frame = []
        for src in chunk.srcs:
            if len(chunks[src].get_surfaces('pos', '助詞')) > 0:
                case_frame.append((
                    chunks[src].get_surfaces('pos', '助詞').pop(),  # 格パターン
