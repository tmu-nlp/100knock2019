        ...
        for j, chunk in enumerate(chunks):
            ...
            if morph and srcs:
                for s in srcs:  # srcsはlistなので
                    josi = chunks[s].check_josi()  # 係り元に助詞があるかをチェック
                    if josi:
                        josi_list.append(josi.base)

            if josi_list:
                josi_list = sorted(josi_list)  # 辞書順にソート
                p_list.append(morph.base + '\t' + '\t'.join(josi_list))

    text = '\n'.join(p_list)
    write_file(text, 'knock45.txt')


if __name__ == "__main__":
    main()

# sort knock45.txt | uniq -c | sort -n -r > all.txt
# grep '^する\s' knock45.txt | sort | uniq -c | sort -n -r > suru.txt
# grep '^見る\s' knock45.txt | sort | uniq -c | sort -n -r > miru.txt
# grep '^与える\s' knock45.txt | sort | uniq -c | sort -n -r > ataeru.txt
