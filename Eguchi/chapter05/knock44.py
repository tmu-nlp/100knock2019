...
placeline = int(input('文字列を入力してください--> '))


for i, chunkitem in enumerate(chunkalllist[placeline]):
    print ('[%s]morphs=%s \t dst[%s] \t srcs[%s]' % (i,  chunkitem.morphs, chunkitem.dst,chunkitem.srcs ) )
    numdst = int(chunkitem.dst)
    if not numdst == -1:
        G.edge(chunkitem.morphs,  chunkalllist[placeline][int(chunkitem.dst)].morphs)
        print("%s\t%s" % ( chunkitem.morphs, chunkalllist[placeline][int(chunkitem.dst)].morphs ))
